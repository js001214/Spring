...
public class DispatcherServlet extends HttpServlet {
	...
	private void process (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (path.equals("/Login.do")) {
			...
			//3. 비즈니스 로직을 처리하는 인터페이스에 dto를 주입 해서 비즈니스 로직을 처리
			
			// UserService user = new UserServiceImpl();
			UserDAO user = new UserDAO();
			UserDTO userD = user.getUser(dto);
			
			//DB의 클라이언트에서 넘긴 ID와 Password를 select해서 그값을 DTO에 담아서 리턴
			System.out.println(userD);
			
			//4. 백엔드의 로직을 모두 처리후 View 페이지로 이동
			if (userD.getId() != null) { //클라이언트에서 전송한 ID와 Pass가 DB의 값과 일치 할때 

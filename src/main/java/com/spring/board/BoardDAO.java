...
public class BoardDAO implements BoardService {
	...
	@Override
	public void insertBoard(BoardDTO dto) {//BoardDTO를 인풋 받는다 dto에 BoardDTO필드 값이 다 들어가있다.
		System.out.println("==> JDBC로 insertBoard() 기능처리 - 시작");
		//Connection 객체를 사용해서 PreparedStatement 객체 활성화
		
		try {
			
			//오류가 발생시 프로그램이 종료되지않도록 try catch 블락으로 처리
			conn = JDBCUtil.getConnection();
			pstmt = conn.prepareStatement(BOARD_INSERT); //board_insert ? ? ? 에 값을 넣는다.
			
			// pstmt의 ?에 변수값을 할당.
			pstmt.setString(1, dto.getTitle());	//get으로 가져온 값을 첫번째 ? 에 할당한다.
	...
	public void updateBoard(BoardDTO dto) {
		...
		try {
			...
			JDBCUtil.close(pstmt, conn); //객체 종료
		}
	}
	...
	try {
		...
		if(rs.next()) {
			do {
				...
				//boardList : ArrayList에 add 메소드를  사용해서 board(DT))를 저장
				boardList.add(board);
